<template>
  <div class="container">
    <div class="header">
      <h1>欢迎使用
        <strong>Vue-Antd</strong>后台管理系统</h1>
      <h2>Use vue and vue-antn-ui</h2>
    </div>
    <div class="main">
      <form class="form-1">
        <p class="field">
        ...
        <p class="submit">
          <button type="submit" name="submit" @click.stop.prevent="handleSubmit">
            <i class="iconfont icon-jiantou-copy" style="font-size:24px;font-weight:bold;"></i>
          </button>
        </p>
      </form>
    </div>
      ...
      axios.post('/user/login', obj) // 将信息发送给后端
        .then((res) => { // axios返回的数据都在res.data里
          if (res.data.success) { // 如果成功
            ...
            message.error(res.data.info) // 登录失败，显示提示语
...
.container {
  ...
  height: 100%;
  width: 100%;
  background: #ddd url(../../../static/bg.jpg) repeat top left;
}
.header {
